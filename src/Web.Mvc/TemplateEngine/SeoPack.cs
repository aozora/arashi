...
namespace Arashi.Web.Mvc.TemplateEngine
{
      ...
      where TModel : TemplateContentModel
   {
      ...
      protected IHtmlString wp_head()
      {
         ...
         {
            keywords = Model.Site.SeoSettings.HomeKeywords.ToEmptyStringIfNull().Trim();
         }
         ...
         if (is_single() || is_page() || is_static_posts_page())
         {
            if (is_static_front_page())
               description = Model.Site.SeoSettings.HomeDescription.ToEmptyStringIfNull().Trim();
            else if (is_page())
             ...
             !(is_home() && is_paged()) )
         {
            ...
            description = descriptionFormat.Replace("%description%", description)
               .Replace("%blog_title%", get_bloginfo("name").ToString())
               .Replace("%blog_description%", get_bloginfo("description").ToString())
      ...
      /// <returns></returns>
      protected IHtmlString wp_title(string sep, bool echo, string seplocation)
      {
         if (is_home() && !is_static_posts_page() )
         {
            string title = Model.Site.SeoSettings.HomeTitle.ToEmptyStringIfNull();

            if (string.IsNullOrEmpty(title))
               title = get_option("blogname");

            return Html.Raw(Html.Encode(title));
         }
         
         if (is_author())
         {
            string titleFormat = Model.Site.SeoSettings.ArchiveTitleFormat.ToEmptyStringIfNull();
            string title = titleFormat.Replace("%blog_title%", get_bloginfo("name").ToString())
               .Replace("%blog_description%", get_bloginfo("description").ToString())
         ...
         if (is_single())
         {
            ...
            string titleFormat = Model.Site.SeoSettings.PostTitleFormat.ToEmptyStringIfNull();
            string title = titleFormat.Replace("%blog_title%", get_bloginfo("name").ToString())
               .Replace("%blog_description%", get_bloginfo("description").ToString())
         ...
         if (is_search() && !string.IsNullOrEmpty(Request.QueryString["s"]))
         {
            ...
            string titleFormat = Model.Site.SeoSettings.SearchTitleFormat.ToEmptyStringIfNull();
         ...
         if (is_category())
         {
            ...
            string titleFormat = Model.Site.SeoSettings.CategoryTitleFormat.ToEmptyStringIfNull();
         ...
         if (is_page() || is_static_posts_page())
         {
            ...
            else
            {
               ...
               string titleFormat = Model.Site.SeoSettings.PageTitleFormat.ToEmptyStringIfNull();
               string newTitle = titleFormat.Replace("%blog_title%", get_bloginfo("name").ToString())
                                             .Replace("%blog_description%", get_bloginfo("description").ToString())
         ...
         if (is_tag())
         {
            ...
            string titleFormat = Model.Site.SeoSettings.TagTitleFormat.ToEmptyStringIfNull();
            string title = titleFormat.Replace("%blog_title%", get_bloginfo("name").ToString())
               .Replace("%blog_description%", get_bloginfo("description").ToString())
               .Replace("%tag%", tag);
         ...
         if (is_404())
         {
            string titleFormat = Model.Site.SeoSettings.Page404TitleFormat.ToEmptyStringIfNull();
            string title = titleFormat.Replace("%blog_title%", get_bloginfo("name").ToString())
               .Replace("%blog_description%", get_bloginfo("description").ToString())
         ...
         return Html.Raw(Html.Encode(Model.Site.SeoSettings.HomeTitle));
      }
      ...
      private string GetOriginalTitle()
      {
         ...
         else if (is_404())
         {
            ...
            title = titleFormat.Replace("%blog_title%", get_bloginfo("name").ToString())
               .Replace("%blog_description%", get_bloginfo("description").ToString())
               .Replace("%request_url%", Request.Url.ToString())
               .Replace("%request_words%", RequestUrlAsWords(Request.Url.ToString()));
         }
