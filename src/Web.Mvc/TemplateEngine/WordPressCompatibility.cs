namespace Arashi.Web.Mvc.TemplateEngine
{
   ...
   public partial class ThemeWebViewPageBase<TModel> : WebViewPage<TModel>
      where TModel : TemplateContentModel
   {
      ...
      /// </summary>
      /// <returns></returns>
      protected IHtmlString wp_title()
      {
      ...
      /// </remarks>
      protected IHtmlString get_bloginfo(string parameter)
      {
         switch (parameter)
         {
            case "home":
               return Html.Raw(GetCurrentSiteUrlRoot());
            case "url":
               return Html.Raw(Request.Url.ToString());
            case "html_type":
               return Html.Raw("text/html");
            case "charset":
               return Html.Raw("iso-8859-1");
            case "template_url":
            case "template_directory":
               return Html.Raw(GetAbsoluteUrl(VirtualPathUtility.ToAbsolute(Model.Site.Theme.BasePath)));
            case "stylesheet_directory":
               return Html.Raw(GetAbsoluteUrl(VirtualPathUtility.ToAbsolute(Model.Site.Theme.BasePath)));
            case "stylesheet_url":
               return Html.Raw(GetAbsoluteUrl(VirtualPathUtility.ToAbsolute(string.Concat(Model.Site.Theme.BasePath, "/style.css"))));
            case "name":
               return Html.Raw(Html.Encode(Model.Site.Name));
            case "description":
               return Html.Raw(Html.Encode(Model.Site.Description));
            case "rss2_url":
               return Html.Raw(GetCurrentSiteUrlRoot() + "/feed/");
            case "atom_url":
               return Html.Raw(GetCurrentSiteUrlRoot() + "/feed/atom/");
            case "pingback_url":
               return Html.Raw(GetCurrentSiteUrlRoot() + "/pingback.axd");
            case "text_direction":
               return Html.Raw("ltr"); //TODO: Implement text_direction!!!
            default:
      ...
      /// <param name="parameter"></param>
      /// <returns></returns>
      protected IHtmlString bloginfo(string parameter)
      {
      ...
      /// </summary>
      /// <returns></returns>
      protected IHtmlString wp_footer()
      {
         // Call the rendering of registered scripts
         return MvcHtmlString.Create(Html.RenderJsHere());
      }
      ...
      protected IHtmlString wp_list_cats(string args)
      {
      ...
      /// <returns></returns>
      protected IHtmlString wp_list_categories(string args)
      {
      ...
      /// <param name="args"></param>
      /// <returns></returns>
      protected IHtmlString wp_get_archives(string args)
      {
         IDictionary<String, String> arguments = GetDictionaryFromQueryStringArray(args);
      ...
      /// </summary>
      /// <returns></returns>
      protected IHtmlString wp_list_pages(string args)
      {
         if (Model.Pages == null)
            return Html.Raw(string.Empty);
      ...
      protected string get_option(string option)
      {
         switch (option)
         {
            ...
            default:
               //SiteOption siteOption = Model.Site.Options.Where(o => o.Name == option).SingleOrDefault();
               //if (siteOption != null)
               //   return siteOption.Value;
               if (Model.Site.Options.ContainsKey(option))
                  return Model.Site.Options[option];
      ...
      protected IHtmlString get_header()
      {
         return Html.Partial(GetViewVirtualPath(ViewHelper.TemplateFile.header));
      }

      protected IHtmlString get_sidebar()
      {
         return Html.Partial(GetViewVirtualPath(ViewHelper.TemplateFile.sidebar));
      }

      protected IHtmlString get_footer()
      {
         return Html.Partial(GetViewVirtualPath(ViewHelper.TemplateFile.footer));
      }



      protected IHtmlString get_search_form()
      {
         return Html.Partial(GetViewVirtualPath(ViewHelper.TemplateFile.searchform));
      }

      protected IHtmlString get_avatar(string email, int size)
      {
     ...
     protected bool is_paged()
      ...
      /// <summary>
      /// Checks if a Category archive page is being displayed. This is a boolean function, meaning it returns either TRUE or FALSE.
      /// </summary>
      /// <returns></returns>
      ...
      /// Checks if a Category archive page is being displayed. This is a boolean function, meaning it returns either TRUE or FALSE.
      /// </summary>
      /// <param name="categoryPageId">(integer) (optional) Category ID, Category Title or Category Slug</param>
      /// <returns></returns>
      protected bool is_category(int categoryPageId)
      {
         return (Model.ViewMode == ViewHelper.ViewMode.is_category) && (Model.CurrentPage != null && Model.CurrentPage.Id == categoryPageId);
      }
      protected bool is_category(string categoryPageId)
      {
         if (string.IsNullOrEmpty(categoryPageId))
            return false;

         return (Model.ViewMode == ViewHelper.ViewMode.is_category) && (Model.CurrentPage != null && Model.CurrentPage.Id == Convert.ToInt32(categoryPageId));
      }
      ...
      protected IHtmlString body_class()
      {
      ...
      protected IHtmlString the_content()
      {
      ...
      /// <returns></returns>
      protected IHtmlString the_content(string more_link_text)
      {
      ...
      /// <returns></returns>
      protected IHtmlString the_content(string more_link_text, bool strip_teaser, string more_file)
      {
         bool hasTeaser = false; // TODO: check in the equivalent WP function if this variable is used!
             ...
             !string.IsNullOrEmpty(more_link_text))
         {
            ...
            else
            {
               ...
               {
                  string teaser = content.Substring(0, content.IndexOf(moreLink));

                  // force_balance_tags to ensure no unclosed tags
                  HtmlAgilityPack.HtmlDocument doc = new HtmlDocument {OptionFixNestedTags = true};
                  doc.LoadHtml(teaser);
...
//}
      ...
      /// </summary>
      /// <param name="cats">int|array $cats The target categories. Integer ID or array of integer IDs</param>
      /// <param name="post">int|object $_post The post. Omit to test the current post in the Loop or main query</param>
      /// <returns>True if at least 1 of the post's categories is a descendant of any of the target categories</returns>
      protected bool post_is_in_descendant_category(string cats, int? post = null )
      {
         string[] _cats;
         ...
         if (cats.IndexOf('&') > -1)
            ...
            _cats = new string[]{cats};

	      foreach ( string cat in _cats) 
         {
            if (currentPost.Categories.Where(c => c.ParentCategory.Id == Convert.ToInt32(cat)).Count() > 0)
               return true;
      ...
      ///// </summary>
      ///// <param name="term">(string) (required) ID of Term to get children. Default: None</param>
      ///// <param name="taxonomy">(string) (required) Taxonomy Name. Default: None</param>
      ///// <returns>List of Term Objects. WP_Error returned if $taxonomy does not exist</returns>
      //protected string get_term_children( int term, string taxonomy )
      //{
      ...
      protected string the_author_link()
      {
         return currentPost.Author.WebSite;
      }
      ...
      // TODO
      protected IHtmlString language_attributes(string doctype = "html")
      {
         return Html.Raw(string.Empty);
        ...
        // HttpBrowserCapabilities.BrowserCapabilitiesProvider.GetBrowserCapabilities().
        //if ( languages.Contains(get_bloginfo("language")) ) {
      ...
      /// <returns></returns>
      protected virtual IHtmlString the_tags(string before, string separator, string after)
      {
      ...
      protected IHtmlString RenderTagsLinks(ICollection<Tag> list, string before, string separator, string after)
      {
         if (list.Count == 0)
            return Html.Raw(string.Empty);
      ...
      /// </summary>
      /// <returns></returns>
      protected virtual ICollection<Category> get_the_category()
      {
      ...
      /// This tag may be used outside The Loop by passing a post id as the parameter
      /// </summary>
      /// <param name="postId">(integer) (optional) The post id. Default: $post->ID (The current post ID)</param>
      /// <returns></returns>
      protected virtual ICollection<Category> get_the_category(int postId)
      {
         Post post = (from p in Model.Posts
                     where p.Id == postId
                     select p).SingleOrDefault();

         return post == null ? null : post.Categories;
      }
      ...
      /// </summary>
      /// <param name="categoryId">The numeric category ID for which to return the parents. Defaults to current category, if one is set.</param>
      protected virtual IHtmlString get_category_parents(int categoryId)
      {
         return get_category_parents(categoryId, false, "/", false);
      }
      ...
      /// </summary>
      /// <param name="categoryId">The numeric category ID for which to return the parents. Defaults to current category, if one is set.</param>
      /// <param name="displaylink">Creates a link to each category displayed. default is false.</param>
      /// <param name="separator">What to separate each category by. default is "/"</param>
      /// <param name="nicename">Return category nice name(slug) or not (defaults to FALSE).</param>
      protected virtual IHtmlString get_category_parents(int categoryId, bool displaylink, string separator, bool nicename = false)
      {
         // TODO: check if property generate the hierarchy
         if (string.IsNullOrEmpty(separator))
            separator = "/";

         List<Category> chain = new List<Category>();
         IEnumerable<Category> firstLevelParent = from c in Model.Categories
                                                  where c.Id == categoryId
                                                  select c.ParentCategory;

         foreach (Category category in firstLevelParent)
         {
            //IEnumerable<Category> flat = from parent in category.AsBreadthFirstEnumerable(c => c.ParentCategory)
            //                             select parent;
            IEnumerable<Category> flat = from parent in category.AsBreadthFirstEnumerable(c => c.ChildCategories)
                                         select parent;
            chain.AddRange(flat);
         }
         ...
         foreach (Category category in chain)
         {
            html.Append("<a href=\"" + category.GetCategoryUrl() + "'\" title=\"" + "View all posts in %s" + Html.Encode(category.Name) + "\">" + category.Name + "</a>" + separator);
         }

         return Html.Raw(html.ToString());


         //Category parent = Model.Categories.Where(c => c.Id == categoryId).SingleOrDefault();
         //if (parent == null)
         //   return null;

         //string name = nicename ? parent.FriendlyName : parent.Name;
      ...
      /// <returns></returns>
      protected virtual IHtmlString the_category(string separator)
      {
         return RenderCategoriesLinks(currentPost.Categories, separator);
      }

      // TODO: manage the parents!!
      protected virtual IHtmlString the_category(string separator, string parents)
      {
         return the_category(separator);
      }

      protected IHtmlString RenderCategoriesLinks(ICollection<Category> list, string separator)
      {
         ...
         return Html.Raw(categories.ToString());
      }
      ...
      /// </summary>
      protected IHtmlString comments_template()
      {
         return Html.Partial(GetViewVirtualPath(ViewHelper.TemplateFile.comments), Model);
      }
      ...
      /// </summary>
      /// <returns></returns>
      protected IHtmlString comment_id_fields()
      {
         return Html.Raw("<input type=\"hidden\" id=\"comment_post_ID\" value=\"" + currentPost.Id.ToString() + "\" name=\"comment_post_ID\"/>");
      }
      ...
      /// <returns></returns>
      protected IHtmlString comments_popup_link(string zero, string one, string more, string cssClass, string none)
      {
      ...
      protected IHtmlString comments_popup_link(string zero, string one, string more)
      {
...
<rdf:RDF xmlns:rdf=""http://www.w3.org/1999/02/22-rdf-syntax-ns#""
   ...
   xmlns:trackback=""http://madskills.com/public/xml/rss/module/trackback/"">
	<rdf:Description rdf:about=""{0}""
      ...
      /// <returns></returns>
      protected IHtmlString next_posts_link(string label, int max_pages)
      {
         ...
         {
            return Html.Raw(GenerateNavigationLink(label, currentPage + 1));
         }

         return Html.Raw(string.Empty);
      }

      protected virtual IHtmlString next_posts_link(string label)
      {
         return next_posts_link(label, 0);
      }

      protected IHtmlString next_posts_link()
      {
      ...
      /// </summary>
      /// <returns></returns>
      protected IHtmlString previous_posts_link(string label, int max_pages)
      {
         if (string.IsNullOrEmpty(label))
         {
      ...
      protected virtual IHtmlString previous_posts_link(string label)
      {
         return previous_posts_link(label, 0);
      }

      protected IHtmlString previous_posts_link()
      {
      ...
      protected IHtmlString wp_pagenavi(string before, string after)
      {
         return Html.Pager(Model.Posts.PageSize, Model.Posts.PageNumber, Model.Posts.TotalItemCount);
      }

      protected IHtmlString wp_pagenavi()
      {
      ...
      protected bool dynamic_sidebar(string name)
      {
         // TODO implement multiple sidebars
         return false;
      }
      ...
      protected bool dynamic_sidebar(int index, string before_widget, string after_widget, string before_title, string after_title)
      {
         ...
         Html.ViewContext.Writer.Write(html.ToString());
      ...
      protected IHtmlString wp_enqueue_style(string handle, string src)
      {
         return wp_enqueue_style(handle, src, string.Empty, "v1", "screen");
      }
      ...
      /// </param>
      protected IHtmlString wp_enqueue_style(string handle, string src, string deps, string ver, string media)
      {
         log.DebugFormat("WordPressCompatibility.wp_enqueue_style: src = {0}", src);

         // get a FileInfo from the full path
         string virtualSrc = string.Concat(VirtualPathUtility.ToAppRelative(Model.Site.Theme.BasePath), src);
         string defaultPath = virtualSrc.Substring(0, virtualSrc.LastIndexOf(Path.AltDirectorySeparatorChar));

         Html.RequiresCss(virtualSrc);
      ...
      /// </summary>
      protected IHtmlString wp_enqueue_style_render()
      {
      ...
      /// Scripts are loaded from the ~/Content/js folder
      /// </summary>
      /// <remarks>
      /// See:
      ...
      /// <param name="order">Order of script rendering: lower are first!</param>
      protected IHtmlString wp_enqueue_script(string handle, int order)
      {
         ...
         // get a FileInfo from the full path
         string virtualSrc = Url.Content("~/Content/js/" + handle + ".js");  //string.Concat(VirtualPathUtility.ToAppRelative(Model.Site.Theme.BasePath), src);
         string defaultPath = virtualSrc.Substring(0, virtualSrc.LastIndexOf(Path.AltDirectorySeparatorChar));

         // get a FileInfo from the full path
         //FileInfo scriptFileInfo = new FileInfo(Context.Server.MapPath(virtualSrc));

         Html.RequiresJs(virtualSrc, order);
      ...
      /// </remarks>
      /// <param name="handle">Name of the script. Lowercase string</param>
      /// <param name="src">relative URL to the script (will be prefixed with Theme.BasePath</param>
      /// <param name="order">Order of script rendering: lower are first!</param>
      protected IHtmlString wp_enqueue_script(string handle, string src, int order)
      {
         return wp_enqueue_script(handle, src, string.Empty, "v1", "true", order);
      }
      ...
      /// <param name="order">Order of script rendering: lower are first!</param>
      /// <returns></returns>
      protected IHtmlString wp_enqueue_script(string handle, string src, string deps, string ver, string in_footer, int order)
      {
