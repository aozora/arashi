...
namespace Arashi.Services.Content
{
   ...
   public class ContentItemService<T> : ServiceBase, IContentItemService<T> where T : IContentItem
   {
      protected IContentItemDao<T> contentItemDao;


      public ContentItemService(Arashi.Core.NHibernate.ISessionFactory sessionFactory, ILog log, IContentItemDao<T> contentItemDao)
          : base(sessionFactory, log)
     {
      ...
      public IList<ContentItemCalendarDTO> GetPostCalendarForPublishedBySite(Site site)
      {
         ...
         IList<ContentItemCalendarDTO> list = Session.GetNamedQuery("GetPostCalendarForPublishedBySite")
                                                   .SetEntity("site", site)
      ...
      public Paginator<T> GetPaginatorBySite(Site site, int pageSize)
      {
         ...
         //   criteria.Add(Restrictions.InsensitiveLike("Title", search, MatchMode.Anywhere));
      ...
      public IEnumerable<T> ConvertSearchResultCollectionToContentItemList(SearchResultCollection results)
      {
         ...
         foreach (SearchResult result in results)
         {
            ...
            var s = list.Select(c => c != null && c.Id == result.ContentItemId).Distinct();
