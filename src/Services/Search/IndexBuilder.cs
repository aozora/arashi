...
namespace Arashi.Services.Search
{
   ...
   public class IndexBuilder : IDisposable
   {
      ...
      private Document BuildDocumentFromContentItem(IContentItem contentItem, ITextExtractor textExtractor)
      {
         ...
         string plainTextContent = contentToIndex.StripHtml();
         // create the actual url
         string path = contentItem.GetContentUrl();
         // check that summary is not null. 
         string summary = string.IsNullOrEmpty(contentItem.Summary) ? Text.TruncateText(plainTextContent, 200) : contentItem.Summary;
         ...
         Field title = new Field("title", contentItem.Title, Field.Store.YES, Field.Index.TOKENIZED);
         title.SetBoost(2.0f);
         doc.Add(title);

         doc.Add(new Field("summary", summary, Field.Store.YES, Field.Index.TOKENIZED));
         doc.Add(new Field("contents", plainTextContent, Field.Store.YES, Field.Index.TOKENIZED));
         doc.Add(new Field("author", contentItem.Author.FullName, Field.Store.YES, Field.Index.TOKENIZED));
         ...
         doc.Add(new Field("site", contentItem.Site.SiteId.ToString(), Field.Store.YES, Field.Index.UN_TOKENIZED));
         doc.Add(new Field("datecreated", contentItem.CreatedDate.ToString("u"), Field.Store.YES, Field.Index.UN_TOKENIZED));
         doc.Add(new Field("datemodified", contentItem.UpdatedDate.ToString("u"), Field.Store.YES, Field.Index.UN_TOKENIZED));
         if (contentItem.PublishedDate.HasValue)
         {
            doc.Add(new Field("datepublished", contentItem.PublishedDate.Value.ToString("u"), Field.Store.YES, Field.Index.UN_TOKENIZED));
         }
         ...
         doc.Add(new Field("contentitemid", contentItem.Id.ToString(), Field.Store.YES, Field.Index.UN_TOKENIZED));

         foreach (Category cat in contentItem.Categories)
         {
            Field categoryField = new Field("category", cat.Name, Field.Store.YES, Field.Index.TOKENIZED);
            categoryField.SetBoost(4.0f);
            doc.Add(categoryField);
            //doc.Add(new Field("category", cat.Name, Field.Store.YES, Field.Index.TOKENIZED));
         }

         foreach (Tag tag in contentItem.Tags)
         {
            Field tagField = new Field("tag", tag.Name, Field.Store.YES, Field.Index.TOKENIZED);
            tagField.SetBoost(4.0f);
            doc.Add(tagField);
            //doc.Add(new Field("tag", tag.Name, Field.Store.YES, Field.Index.TOKENIZED));
         }

         foreach (Role viewRole in contentItem.ViewRoles)
         {
            doc.Add(new Field("viewroleid", viewRole.RoleId.ToString(), Field.Store.YES, Field.Index.UN_TOKENIZED));
         }
      ...
      /// <summary>
      /// Optimize the index.
      /// This method should be called before the index close.
      /// </summary>
