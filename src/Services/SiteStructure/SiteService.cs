...
namespace Arashi.Services.SiteStructure
{
   ...
   public class SiteService : ISiteService
   {
      ...
      public SiteService(IUserService userService, IThemeService themeService, IFileService fileService)
      {
      ...
      public Site CreateNewSite(string name, string description, string email, string siteHost)
      {
         // Site
         Site site = new Site
         {
         ...
         // Template
         Template defaultTemplate = themeService.GetById(0);
         site.Template = defaultTemplate;
      ...
      public Site GetSiteById(int siteId)
      {
         using (NHTransactionScope tx = new NHTransactionScope())
         {
            Site site = RepositoryHelper.GetSession().GetNamedQuery("GetSiteById")
      ...
      {
         return RepositoryHelper.GetSession().GetNamedQuery("GetSiteByHostName")
                        .SetString("host", host)
                        .UniqueResult<Site>();
      }
      ...
      {
         return RepositoryHelper.GetSession().GetNamedQuery("GetSiteHostsBySite")
                        .SetEntity("site", site)
                        .List<SiteHost>();
      }
      ...
      {
         return RepositoryHelper.GetSession().GetNamedQuery("GetSiteHostByHostName")
                        .SetString("host", host)
      ...
      {
         return Repository<Template>.FindById(templateId);
      }


      public IList<Template> GetTemplates()
      {
         return RepositoryHelper.GetSession().GetNamedQuery("GetTemplates")
                        .List<Template>();
      }
      ...
      public IList<TrackingInfo> GetTrackingInfoForUsersOnline(int minutesSinceLastInactive)
      {
         ...
         return RepositoryHelper.GetSession().GetNamedQuery("GetTrackingInfoForUsersOnline")
      ...
      //public IList<TrackingInfo> GetNumberOfUsersOnline(int minutesSinceLastInactive)
      //public long GetNumberOfUsersOnline(int minutesSinceLastInactive)
      //{
      //   DateTime lastActivityDate = DateTime.Now.AddMinutes(-minutesSinceLastInactive).ToUniversalTime();

      //   //string hql = "select count(t) from (select distinct t.UserIP from TrackingInfo t where t.TrackingDate > :lastActivityDate)";
      //   string hql = "select count(distinct t.UserIP) from TrackingInfo t where t.TrackingDate > :lastActivityDate";

      //      //.SetResultTransformer(new DistinctRootEntityResultTransformer())
      //   return RepositoryHelper.GetSession().CreateQuery(hql)
      //      .SetDateTime("lastActivityDate", lastActivityDate)
      //      .UniqueResult<long>();
      //}
