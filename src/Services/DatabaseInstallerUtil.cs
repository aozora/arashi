...
namespace Arashi.Services
{
   ...
   public class DatabaseUtil
   {
      ...
      public static Version GetAssemblyVersion(string assembly)
      {
         ...
         string sql = String.Format("SELECT major, minor, patch FROM cms_Versions WHERE assembly = '{0}'", assembly);
         ...
         try
         {
            ...
            version = dr.Read() ? new Version(Convert.ToInt32(dr["major"]), Convert.ToInt32(dr["minor"]), Convert.ToInt32(dr["patch"])) : new Version(0, 0, 0);
      ...
      {
         // NOTE: this must be changed if I must manage multiple db connections via NH
         //return IoC.Resolve<ISessionFactoryImplementor>();
         return IoC.Resolve<ISessionFactoryImplementor>("nhibernate.factory");

         //ISessionBuilder sb = IoC.Resolve<ISessionBuilder>();
         //return sb.GetSessionFactory() as ISessionFactoryImplementor;
      }
