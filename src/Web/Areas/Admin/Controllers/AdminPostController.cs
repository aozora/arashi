...
namespace Arashi.Web.Areas.Admin.Controllers
{
   ...
   public class AdminPostController : SecureControllerBase
   {
      ...
      public AdminPostController(ILog log, IContentItemService<Post> contentItemService,
      ...
      public ActionResult Index(int? page, string status, string s)
      {
         ...
         if (!string.IsNullOrEmpty(s))
         {
            ...
            posts = contentItemService.ConvertSearchResultCollectionToContentItemList(results);

            pagedList = new PagedList<Post>(posts, page.HasValue ? page.Value - 1 : 0, pageSize, results.TotalCount);// , totalcount
         }
         else
         {
      ...
      public ActionResult SaveNew([Bind(Exclude = "Id")] Post post, int[] categoryid, int[] tagid, int month, int day, int year, int hour, int minute)
      {
         try
         {
            UpdateModel(post, new[] { "Title", "Summary", "Content", "WorkflowStatus", "AllowComments", "AllowPings", "EnableMeta", "MetaDescription", "MetaKeywords" });
      ...
      public ActionResult Edit(int id)
      {
         ...
         Post model = contentItemService.GetById(id);
         IEnumerable<Category> list = categoryService.GetAllCategoriesBySite(Context.ManagedSite);

         ViewBag.SiteCategories = list;
         //ViewBag.SiteCategoriesSelectList = new SelectList(list, "Id", "Name");
         ViewBag.SiteTags = tagService.GetAllTagsBySite(Context.ManagedSite);

         // Get the full WorkflowStatus enum list, and remove the current page status value:
         IDictionary<String, String> statusList = GetLocalizedEnumList(typeof(WorkflowStatus));
         //ViewBag.WorkflowStatusSelectList = new SelectList(statusList, "Key", "Value");
      ...
      /// <param name="hour"></param>
      /// <param name="minute"></param>
      /// <param name="WorkflowStatusList"></param>
      /// <returns></returns>
      [PermissionFilter(RequiredRights = Rights.PostsEdit)]
      [AcceptVerbs(HttpVerbs.Post)]
      [ValidateInput(false)]
      [ValidateAntiForgeryToken]
      public ActionResult Update(int id, int[] categoryid, int[] tagid, int month, int day, int year, int hour, int minute, string WorkflowStatusList)
      {
         Post post = contentItemService.GetById(id);

         try
         {
            UpdateModel(post, new[] { "Title", "Summary", "Content", "AllowComments", "AllowPings", "EnableMeta", "MetaDescription", "MetaKeywords" });

            post.PublishedDate = new DateTime(year, month, day, hour, minute, 0);

            if (!string.IsNullOrEmpty(WorkflowStatusList))
               post.WorkflowStatus = (Arashi.Core.Domain.WorkflowStatus)Enum.Parse(typeof(Arashi.Core.Domain.WorkflowStatus), WorkflowStatusList);
