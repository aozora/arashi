namespace Arashi.Web.Areas.Admin.Controllers
{
   ...
   public class AdminContentItemController : SecureControllerBase
   {
      ...
      #region ContentItems AutoComplete support

      [HttpPost]
      public JsonResult FindContentItems(string searchText, int maxResults)
      {
         // TODO: create new method for filtering
         var items = (from i in contentItemService.FindAllBySite(Context.ManagedSite)
                      where i.Title.Contains(searchText)
                      select new { Id = i.Id, Title = i.Title, Url = i.GetContentUrl() })
                                           .Take(maxResults);
         return Json(items);
      }
