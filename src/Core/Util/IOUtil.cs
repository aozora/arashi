...
namespace Arashi.Core.Util
{
   ...
   {
      private static readonly ILog log = LogManager.GetLogger(typeof(IOUtil));
      ...
      {
         log.Debug("IOUtil.CheckIfDirectoryIsWritable: path to check: " + physicalDirectory);
         ...
         {
            log.Debug("IOUtil.CheckIfDirectoryIsWritable: path is not accessible!");
            return false;
         }
      }
